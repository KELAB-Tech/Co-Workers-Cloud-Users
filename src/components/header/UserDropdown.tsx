...
function AvatarCircle({ profile, avatarUrl }: { profile: UserProfile | null; avatarUrl: string | null }) {
  if (!profile) {
    return (
      <span className="flex items-center justify-center overflow-hidden rounded-full h-11 w-11 bg-gray-100 dark:bg-gray-800 animate-pulse" />
    );
  }
...
export default function UserDropdown() {
  ...
  useEffect(() => {
    ...
    getMyProfile()
      .then(setProfile)
      .catch(() => { /* silencioso — no interrumpir el header */ });
  }, []);
  ...
  const handleLogout = () => {
    ...
    Cookies.remove("actorType");
    router.push("/signin");
  };

  // Nombre corto para el header (primer token)
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3">
          <AvatarCircle profile={profile} avatarUrl={avatarUrl} />
        </span>
